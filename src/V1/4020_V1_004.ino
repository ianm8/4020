...
void update_display(const uint32_t signal_level = 0u)
{
    ...
    if (radio.tx_enable)
    {
      ...
      oled.print(radio.mic_proc?"PR":"TX");
...
void loop1(void)
{
  ...
  if (!radio.tx_enable)
  {
    ...
    switch (button_action)
    {
      ...
      {
        radio.attenuation = !radio.attenuation;
        digitalWrite(PIN_ATT,radio.attenuation?HIGH:LOW);
        break;
      }
    }
    ...
    if (radio.frequency!=old_frequency)
    {
      ...
      // set the mode based on frequency
      radio.mode = MODE_USB;
      ...
      switch (tx_button_state)
      {
        case TX_BUTTON_IDLE:
        {
          if (digitalRead(PIN_ENCBUT)==LOW)
          {
            ...
            // toggle mic processing
            radio.mic_proc = !radio.mic_proc;
            update_display(mic_peak_level);
