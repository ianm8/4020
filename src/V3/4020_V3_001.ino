  ...
  if (tx)
  {
    ...
    if (radio.tx_enable)
    {
      if (adc_value_ready)
      {
        ...
        {
          process_mic(adc_value,tx_i,tx_q,radio.mic_proc);
        }
...
void loop1(void)
{
  ...
  if (!radio.tx_enable)
  {
    ...
    else if (radio.set_mode==SETMODE_CWL)
    {
      radio.mode = MODE_CWL;
    }
    else if (radio.set_mode==SETMODE_CWU)
    {
      radio.mode = MODE_CWU;
    }
    ...
    // reset phase if mode has changed
